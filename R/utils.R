...
.regionStringToGRanges <- function(regions,
    ...
    if (is.null(seqinfo)) {
        ...
        if (is.numeric(seqinfo) && !is.null(names(seqinfo))) {
            ...
            if (supportsSeqlengths) {
                ...
                cli_abort(
                    paste0(
                        "{.arg seqinfo} must be {.code NULL}, ",
                        "an object supporting {.fn seqlengths} or ",
                        "a named {.cls numeric} vector with genomic sequence lengths."))
            }
        }
    }
    ...
    if (any(regions == ".")) {
        if (length(regions) != 1L) {
            cli_abort("{.code regions='.'} can only be given as a single region")
            ...
            cli_abort(
                paste0("For {.code regions='.'} a {.arg seqinfo} argument is ",
                       "required that supports {.fn seqlengths}"))
        }
    } else {
