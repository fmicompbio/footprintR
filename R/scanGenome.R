...
#' @param se A \code{\link[SummarizedExperiment]{RangedSummarizedExperiment}}
#'     object, as generated by \code{readModBam(..., level = "summary")}
#'     (i.e. with the assay \code{"FracMod"}).
#' @param gr A \code{\link[GenomicRanges]{GRanges}} object defining the windows
...
#' @importFrom SummarizedExperiment rowRanges colData assay assayNames
#' @importFrom GenomicRanges GPos
#' @importFrom IRanges findOverlaps width start
#' @importFrom S4Vectors queryHits subjectHits metadata
#' @importFrom cli cli_abort
#'
#' @export
phasingScoreFourier <- function(se, gr, numCoef = 5) {
    ...
    if (!all(c("FracMod") %in% assayNames(se))) {
        cli_abort("{.arg se} must contain assay FracMod")
    }
    ...
    if (nrow(se) > 0) {
        ...
        # Fourier transform
        fit <- e1071::stft(X = fracNucl, win = (numCoef - 1) * period, inc = period,
                           coef = 2 * period, wtype = "hanning.window")

        # Extract coefficient of ineterst
        pd2 <- data.frame(window_mid = (2 * period + (seq.int(nrow(fit$values)) - 1) * period) + start(regZoom) - 1,
                          nucl_score = fit$values[, 4 + 1])
